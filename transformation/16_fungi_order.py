...
print("2. Determine the last column to generate Order_New entries")
last_column = ws.max_column
output_column = last_column + 1
print(output_column)

print("3. Determine the Order column")
for column in range(last_column, 0, -1):
    cell = ws.cell(row=1, column=column)
    if cell.value == "Order":
        column_order = ws.cell(row=1, column=column).column
        print(column_order)
        break
...
for row in range(2, nr_items + 1):
    ...
    if pot_name != garbage_keyword:
        ...
        class_name = get_val(column - 1, row)
        if class_name != garbage_keyword:
            set_val(output_column, row, class_name + "_ord", replaced)
        else:
            phylum_name = get_val(column - 2, row)
            if phylum_name != garbage_keyword:
                set_val(output_column, row, phylum_name + "_ord", replaced)
            else:
                set_val(output_column, row, "Fungi_ord", replaced)
