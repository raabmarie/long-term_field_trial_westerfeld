...
# folder path
path = "Datenstruktur/Nmin/done/3"
        ...
        for worksheet in workbook.worksheets:
            if worksheet.title.strip() in ["NFK1", "NFK2", "Nmin Ernte", "Nmin Frühj"]:
                ...
                for row in cell_range:
                    for cell in row:
                        ...
                        if isinstance(cell_value, str):
                            # The content needs to be split, because 3 values were added to one cell
                            numbers = [
                                float(cell.replace(",", "."))
                                for cell in cell_value.split()
                            ]

                            # Add new column in case not all 3 cells to the right are empty
                            if (
                                (
                ...
                for row in range(1, ws_keys.max_row + 1):
                    ...
                    worksheet.cell(row=row, column=5).value = valueC

                    valueE = ws_keys.cell(row=row, column=5).value
                    worksheet.cell(row=row, column=14).value = valueE

                    valueG = ws_keys.cell(row=row, column=7).value
                    worksheet.cell(row=row, column=18).value = valueG
