...
print("2. Determine the last column to generate Class_New entries")
last_column = ws.max_column
output_column = last_column + 1
print(output_column)

print("3. Determine the Class column")
for column in range(last_column, 0, -1):
    cell = ws.cell(row=1, column=column)
    if cell.value == "Class":
        column_class = ws.cell(row=1, column=column).column
        print(column_class)
        break
...
for row in range(2, nr_items + 1):
    ...
    if pot_name != garbage_keyword:
        ...
        phylum_name = get_val(column - 1, row)
        if phylum_name != garbage_keyword:
            set_val(output_column, row, phylum_name + "_class", replaced)
        else:
            set_val(output_column, row, "Fungi_class", replaced)
