...
# Open Excel file
workbook = load_workbook("2020_Genexpression_Mais_dct.xlsx")

# Access worksheet dct
ws_dct = workbook["dct"]
...
for row in range(2, 34):
    ...
    # Step 3b): Save the values from K1 to AW1 in a list
    header_values_Gene = []  # 1 x 39 list
    for column in range(11, 50):  # columns K to AW
        cell_value = ws_dct.cell(row=1, column=column).value
        header_values_Gene.append(cell_value)

# Step 3c): Save the matrix K2-AW33 from "dct"
matrix_values_Werte = np.empty((32, 39))  # 32 x 39 matrix
for row in range(2, 34):
    for column in range(11, 50):  # column K to AW
        cell_value = ws_dct.cell(row=row, column=column).value
        matrix_values_Werte[row - 2, column - 11] = cell_value

# Step 4: For 39 genes, steps 5-7 must be executed
for idx_Gene in range(39):
    start_row = 2 + (
        idx_Gene * 32
    ...
    for i, row_values in enumerate(matrix_values_Parzelle):  # iterate over all 32 rows
        for j, cell_value in enumerate(row_values):  # iterate over all 10 columns
...
# Save file
workbook.save("2020_Genexpression_Mais_dct.xlsx")
print("done")
