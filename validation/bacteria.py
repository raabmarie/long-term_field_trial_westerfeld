...
df_bacteria = pd.merge(df_bacteria, df_bioProject[['BioProject_ID', 'BioProject_Name']], on='BioProject_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_habitat[['Habitat_ID', 'Habitat']], on='Habitat_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_kingdom[['Kingdom_ID', 'Kingdom_Name']], on='Kingdom_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_phylum[['Phylum_ID', 'Phylum_Name']], on='Phylum_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_class[['Class_ID', 'Class_Name']], on='Class_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_family[['Family_ID', 'Family_Name']], on='Family_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_order[['Order_ID', 'Order_Name']], on='Order_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_genus[['Genus_ID', 'Genus_Name']], on='Genus_ID', how='left')
df_bacteria = pd.merge(df_bacteria, df_species[['Species_ID', 'Species_Name']], on='Species_ID', how='left')

# Remove and rename columns
df_bacteria.drop(columns=['Bakterien_ID', 'BioProject_ID','Habitat_ID','Kingdom_ID', 'Phylum_ID', 'Class_ID', 'Family_ID', 'Order_ID', 'Genus_ID', 'Species_ID', 'Sample_Name', 'Internal_ID'], inplace=True)
df_bacteria.rename(columns={
    'Versuchsjahr': 'Year',
    'Termin': 'Date',
...
df_bacteria = df_bacteria.sort_values(by=['Identifier', 'Date'], ascending=[True, False]).reset_index(drop=True)
