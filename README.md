...
results of our data quality assurance measures.
...
DQ-Kit is a Web service for data quality purposes. Here, we "only" provide the reports for documentation purposes.
We mainly checked these results for abnormalities and duplicates in the dataset.

### Manually checking for data duplicates

Directory `validation/manual_duplicates`:

These source code files check the bacteria and fungi data records for data duplicates. We used this manual approach
because the data couldn't be processed by DQ-Kit due to dataset size limits.
